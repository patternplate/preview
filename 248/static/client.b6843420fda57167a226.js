...
function Code(props) {
  ...
  var highlights = Array.isArray(props.highlights) ? props.highlights : [];
  var ranges = highlights.reduce(function (r, highlight, index) {
    var previous = r[r.length - 1];

    if (!previous || previous.start + previous.length !== highlight) {
      return _toConsumableArray(r).concat([{
        start: highlight,
        length: 1
      }]);
    }
  ...
  return props.block ? React.createElement(StyledPre, null, code, highlights.length > 0 && React.createElement(StyledLines, null, ranges.map(function (range) {
    return React.createElement(StyledRange, {
      start: range.start,
      length: range.length
    });
  }))) : code;
...
/******/ });
//# sourceMappingURL=client.b6843420fda57167a226.js.map