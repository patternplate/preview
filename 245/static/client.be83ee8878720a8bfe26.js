...
function Code(props) {
  var source = highlightCode(props.children, {
...
function highlightCode() {
  ...
  var _ref = arguments.length > 1 ? arguments[1] : undefined,
      ...
      _ref$highlightedLines = _ref.highlightedLines,
      highlightedLines = _ref$highlightedLines === void 0 ? [] : _ref$highlightedLines;
...
function highlight(source, _ref2) {
...
function toElements(children, _ref3) {
  var highlightedLines = _ref3.highlightedLines;
  ...
  var root = toh(React.createElement, {
    type: "element",
    tagName: "div",
    children: children.reduce(function (lines, child) {
      ...
      if (highlightedLines.includes(lines.length) && !line.properties.className.includes('highlighted-line')) {
        line.properties.className += ' highlighted-line';
      } // TODO: Handle multiline tokens properly


      if (hasNewLine(child)) {
        var _children = afterNewLine(child);
...
function hasNewLine(node) {
  ...
  return node.children.some(function (child) {
    return hasNewLine(child);
  });
}

function strip(node, sign) {
  if (node.type === 'text') {
    node.value = node.value.split(sign).join('');
    return;
  }

  node.children.forEach(function (child) {
    return strip(child, sign);
  });
}

function split(node, sign) {}
...
var rangeParser = __webpack_require__(!(function webpackMissingModule() { var e = new Error("Cannot find module \"parse-numeric-range\""); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
...
function MarkdownCodeBlock(props) {
  ...
  var highlightedLines = getHighlightedLines(props.children);
...
function getLanguages(children) {
  ...
  return className.split(" ").map(function (n) {
    return n.replace("language-", "").replace(/\{([0-9|\,|\-]*)\}/, "");
  }).find(function (n) {
    return typeof n === "string" && n.length > 0;
  }).split(":");
}

function getHighlightedLines(children) {
  ...
  var raw = className.match(/\{([0-9|\,|\-]*)\}/);

  if (!raw) {
    return [];
  }

  return rangeParser.parse(raw[1]);
}
...
var _default = (0, _promiseThunkAction.createPromiseThunkAction)("LISTEN", function (payload, dispatch, getState) {
  ...
  ws.onMessage(async function (envelope) {
    ...
    switch (type) {
      ...
      case "start":
        {
          ...
          return dispatch((0, _loadPatternDemo.default)({
            force: false
          }));
        }
      ...
      case "change":
        {
          return dispatch((0, _loadSchema.default)());
        }
...
var _default = function _default(payload) {
  return async function (dispatch, getState) {
    ...
    var response = await (0, _isomorphicFetch.default)(uri, {
      headers: {
        Accept: "text/html"
      }
    ...
    var body = await response.text(); // Bail if the src changed in the meantime
...
var _default = (0, _promiseThunkAction.createPromiseThunkAction)("LOAD_SCHEMA", async function (_, __, getState) {
  var response = await (0, _fetch.default)("".concat(prefix(getState().base), "/api/state.json"));
...
var asyncIdent = async function asyncIdent(i) {
  return i;
};
...
function createPromiseThunkAction(name, rawCreator) {
  ...
  var fn = function fn(payload) {
    ...
    return async function (dispatch, getState) {
      ...
      var delayedTimer = global.setTimeout(function () {
        dispatch(delayedAction(payload, ident, getState));
      }, 1000);

      try {
        var result = await Promise.resolve(creator(payload, dispatch, getState));
...
function reload() {
  return async function (dispatch) {
    var actions = [async function () {
      return dispatch((await Promise.resolve((0, _.loadSchema)())));
    }, (0, _.loadPatternDemo)({
      force: true
    })];
...
async function main() {
  var slot = document.querySelector('[data-application-el="patternplate"]');
  var vault = document.querySelector('[data-application-state="patternplate"]');
  var data = await getData(vault);
...
async function getData(vault) {
  var data = ARSON.parse(decodeURIComponent(vault.textContent));
  var schema = await getStateData(data.base);
...
async function getStateData(base) {
  return (await fetch("".concat(prefix(base), "/api/state.json"), {
    credentials: "include"
  })).json();
}
...
/******/ });
//# sourceMappingURL=client.be83ee8878720a8bfe26.js.map